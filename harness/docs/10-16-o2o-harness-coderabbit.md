...
최초 작성: 2026-09-10
최종 갱신: 2026-09-11 (4절과 6-1절. 제외를 폴더 단위에서 파일 단위로. 이슈 100)
...
| harness/out/ 아래 README.md 아닌 전부 | 생성 후보와 형식 보정 이력이다. 반영판은 document/로 가므로 거기서 본다 |
...
| 옛 규칙이 막던 파일 | 126개 |
| 새 규칙이 막는 파일 | 122개 |
| 풀린 파일 | 4개. harness/out과 harness/out/import와 harness/reviews와 harness/state의 README.md |
| 새로 막힌 파일 | 0개 |

harness/out의 패턴 하나는 부정 글롭을 쓴다. !harness/out/**/!(README.md)다. 앞의 느낌표는 제외이고 괄호의 느낌표는 minimatch의 부정이라 뜻이 다른 둘이 한 줄에 있다. 처음에는 !harness/out/*/**로 하위 폴더 안쪽만 막았는데 그러면 harness/out/import/README.md처럼 하위 폴더의 색인이 다시 숨는다. README는 어디에 있든 색인이라는 원칙에 어긋난다.

부정 글롭은 minimatch 기본값에서 켜져 있고 그 상태로 실측했다. 봇이 그 옵션을 끄고 돌리는지는 문서에 없다. 다만 안 먹으면 그 줄이 아무것도 안 막아 harness/out 전체가 리뷰에 쏟아진다. 조용히 숨는 쪽이 아니라 시끄럽게 드러나는 쪽이라 다음 PR에서 바로 보인다. 그때는 !harness/out/*/**로 되돌리고 하위 폴더 색인은 포기한다.
...
2026-09-11 실측이다. 옛 규칙 셋과 새 규칙 여섯을 저장소 파일 364개에 각각 걸었다.
