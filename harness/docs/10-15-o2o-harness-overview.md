...
청록과 회색을 가르는 선이 블라인드 평가의 경계다. 회색 다섯에는 왜 그렇게 만들었는지가 적혀 있어서, 평가자가 읽으면 답안지를 먼저 보는 셈이 된다.
...
세 SVG는 스스로 완결된 파일이다. 색과 글꼴과 좌표가 파일 안에 다 들어 있어서 다른 스타일시트가 없어도 그대로 열린다. 대신 아래 셋을 지킨다.

| 항목 | 규칙 |
|---|---|
| 색 | 상자 하나에 배경과 테두리와 제목과 부제 넷을 같은 계열로 맞춘다. 배경이 밝고 글자가 어둡다 |
| 상자 폭 | 한글은 14px에서 글자당 약 15px, 12px에서 약 13px로 잡는다. 부제가 상자 안쪽 폭을 넘으면 부제를 줄인다 |
