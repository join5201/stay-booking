...
| 미해결 사항과 다음 작업 | T1부터 T10이 끝났다. 다음은 프론트 평가 라운드(5절 입력으로 A와 B. 프론트 평가 축 이슈 D-2는 하네스 세션 몫이고 아직 없다)와 사용자의 최종 완료 판단. PR 스택이 열려 있다(T5 PR 164, T6 PR 168, T7 PR 171, T8 PR 173, T9 PR 176, T10 PR은 이 기입 뒤. 각 PR의 base는 앞 PR의 브랜치라 앞에서부터 차례로 병합하고 병합될 때마다 다음 PR의 base를 main으로 바꾼다). E2E 환경은 백엔드(o2o_web_test, hold-ttl 20초)와 next start를 켜 둔 채이고 평소 백엔드(o2o_catalog_test)는 껐다. 평소 백엔드가 필요하면 E2E 백엔드를 끄고 환경 변수 없이 bootRun. 남은 평가 네 쌍의 반영이 백엔드를 바꾸면 2-1절과 4절 해시 갱신 |
