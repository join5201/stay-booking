...
## 4. 입력과 적용 규칙 (2026-09-16 T10 뒤 해시 둘 갱신. 그 앞서 T5와 T3에서 갱신)
...
## 5. A와 B 평가 허용 입력 (HR1) (2026-09-16 T10 평가 대상 코드 행 기입)

프론트 평가는 한다(2026-09-12 결정). 라운드는 T10이 끝난 뒤 한 번이다. 이 표는 그때 넘길 것이다.

| 자료 | 경로 | 버전 또는 해시 | 읽을 범위 |
|---|---|---|---|
| 평가 대상 코드 | harness/out/task-S9-frontend-R1/eval-target-files.md | sha256:8093e745fa22f695 | 목록의 148개 파일 전문(frontend/ 추적 파일 전부. 기준 커밋 0a1f109. package-lock.json은 판 대조만). 버전 칸은 그 목록 파일의 해시이고 파일마다의 sha256은 목록 안에 있다(2026-09-16 T10 기입) |
...
| 미해결 사항과 다음 작업 | T1부터 T10이 끝났다. 다음은 프론트 평가 라운드(5절 입력으로 A와 B. 프론트 평가 축 이슈 D-2는 하네스 세션 몫이고 아직 없다)와 사용자의 최종 완료 판단. PR 스택(T5 PR 164, T6 168, T7 171, T8 173, T9 176, T10 183)은 2026-09-16 18:0x부터 19:0x에 앞에서부터 차례로 병합됐다(164부터 176은 사용자, 183은 사용자 지시로 이 세션이 이력 확인 뒤). 병합될 때마다 다음 PR의 base를 main으로 바꿨고 병합 행은 4-1절 7단계대로 다음 브랜치에 얹었다(173과 176의 행은 183에). 남은 것: frontend/README.md의 test:e2e 줄이 T10 전 문구다. 평가 대상 목록의 해시를 지키려고 안 고쳤고 평가 라운드 준비 때 목록 재생성과 함께 고친다. E2E 환경은 백엔드(o2o_web_test, hold-ttl 20초)와 next start를 켜 둔 채이고 평소 백엔드(o2o_catalog_test)는 껐다. 평소 백엔드가 필요하면 E2E 백엔드를 끄고 환경 변수 없이 bootRun. 결제 R1 반영(PR 175)과 08-3 결정 착지(02 v9)가 T10 뒤 main 병합으로 들어와 4절 해시 둘을 갱신했다. 결제 반영은 PaymentApplicationService와 모의 이벤트 저장소이고 API 형태는 그대로라 2-1절은 안 바뀐다. 02 v9와의 화면 문구 대조는 안 했다. 남은 평가 세 쌍의 반영이 백엔드를 바꾸면 2-1절과 4절 해시 갱신 |
