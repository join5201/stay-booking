# 작업 계약 task-S9-frontend (프론트 화면 전체. 초안)

최초 작성: 2026-09-15
최종 갱신: 2026-09-15 (초안. 인계 문서 HANDOFF.md를 받아 화면과 API 대응표를 옮기고 백엔드 실물과의 차이 여섯을 2-1절에 고정)
...
| 항목 | 내용 |
|---|---|
| Task ID와 Step | task-S9-frontend. Step 9 구현의 프론트 몫. 10-6 E절(프론트 개발)과 F절(연결 테스트) 전부. 이름은 잠정이고 7절 D-1에서 사용자가 정한다 |
| 작업 유형 | 코드 |
...
| 예약 컨트롤러 | backend/src/main/java/com/o2o/booking/api/BookingController.java | sha256:33de44b118ea1e93 | 86행부터 200행. Idempotency-Replayed, PAY-01 202, BOOK-02 status |
...
| 백엔드 실행 안내 | backend/README.md | sha256:7b1d181476324fe9 | 1절부터 4절. 판, 준비, 실행, 포트 |
...
| 지역 코드 | 프론트 상수 열일곱(RegionRegistry와 같은 값). 고정 선택 상자. G1, H2, O2가 같은 상자 | 2-1절 6행. 계약 승인으로 확정 |
...
추천은 가다. N9의 뜻(하나가 검증까지 내려간 뒤 다음)은 8절의 단계 단위로 지킨다. 이름은 잠정이고 사용자가 정한다. tasks README의 기능키 여섯은 백엔드 묶음 이름이라 프론트에는 그대로 맞지 않는다.
...
|---|---|
| 작업 계약 승인 | 대기. 사용자 |
| 마지막 성공 단계 | 계약 초안(2026-09-15). 인계 문서 원문을 received/에 보관 |
| 미해결 사항과 다음 작업 | D-1부터 D-7 사용자 판단. 남은 평가 네 쌍의 반영이 백엔드를 바꾸면 2-1절과 4절 해시 갱신. 승인 뒤 다음은 T1 |
| 최종 산출물과 버전 | 작업 후 기록 |
| 실제 사용 시간 | 계약 초안 미측정 |
| 최종 완료 판단 | 대기. 사용자 |
