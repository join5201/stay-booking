...
최초 작성: 2026-09-11
최종 갱신: 2026-09-15 (개정 4. R1 평가 반영. 0절 신설, 5절 R2 허용 입력 하나, 8-1의 K26과 K27, 10절 개정 4 행과 마지막 성공 단계 행. 그 전 2026-09-12 개정 1부터 3 반영. 2절 검사 순서 표와 응답 모델 필드 수, 2-1절 I3, 8-1절 K13, 10절. 사용자 수용 join5201, 2026-09-12)
...
## 0. 개정 4 (2026-09-15 신설. R1 평가 반영)

개정 근거: 라운드 mvp-eval-2026-09-14 쌍 4(예약 1차와 2차 합본)의 블라인드 평가 R1에서 지적 6건이 나왔고 사용자가 2026-09-15 결정표 초안대로 확정했다(수용 5, 거부 1, 반박 0. 발언은 초안대로 반영해라). 근거는 harness/decisions/task-S9-booking-R1.md다. 이 계약(1차)에 걸리는 것은 A-01(멱등 실행기 검사 순서), A-03(PageResponse 사본), B-01(재고 version)과 거부한 A-02(미정의 Query)다. B-02(승인과 만료의 기준 시점)는 2차 계약 개정 11이고 A-04(테스트 재실행)는 실행만이다. 반영 기록은 harness/out/task-S9-booking-R1/applied/apply-report.md다.

### 0-1. 고치는 곳

| 번호 | 고친 곳 | 무엇에서 무엇으로 |
|---|---|---|
| 1 | 5절 허용 입력 | R2부터 읽는 행 하나. shared의 목록 응답 모델(A-03의 근거. 평가자가 대상 목록 밖이라 못 봤다) |
| 2 | 8-1절 테스트 목록 | K26과 K27 두 행. 단계 칸은 R1 반영. K26 재고 version 증가(B-01), K27 처리 중 다른 body의 IDEMPOTENCY_KEY_REUSED(A-01). K22에 선점 전 version의 VERSION_CONFLICT 반례가 붙었다 |
| 3 | 10절 | 개정 4 행과 마지막 성공 단계 행 |

### 0-2. 개정하지 않은 것

| 항목 | 왜 |
|---|---|
| 2절 검사 순서 표 4행 | 계약은 처음부터 body 다름의 409를 진행 중의 409보다 앞에 적었다. 틀린 것은 구현이었고(완료 여부를 먼저 봄) 실행기의 순서를 바꿔 계약대로 됐다 |
| 2절 BOOK-02 | A-02는 거부다. 11 공통 규칙 74행은 표시하지 않은 Query 필드를 지원하지 않는다고만 적고 상태 코드를 정하지 않으며 400은 38행의 JSON body 몫이다. BOOK-02 처리 규칙의 guestId와 hostId를 받지 않는다는 문장은 본인 예약만 반환한다의 짝이고 K24가 그것을 고정한다. 미정의 Query의 400은 목록 API 전부에 걸리는 명세 결정이라 별도 이슈 |
| 3절 변경 허용 범위 | DailyInventory는 처음부터 이 계약의 변경 허용 범위였다(3절 표). B-01의 반영이 그 안이다. catalog/api의 PageResponse 사본은 범위 밖이라 그대로 두고 10절 미해결 사항이 적은 별도 이슈로 남긴다 |
| 대상 API 셋과 내부 처리 하나 | 그대로. API가 늘거나 줄지 않았다 |
...
## 5. A와 B 평가 허용 입력 (HR1) (2026-09-12 평가 대상 코드 행 기입. 2026-09-15 개정 4 R2 행 하나)

오늘의 전제 결정 3에 따라 평가는 MVP 코드가 다 붙은 뒤 한 번이다. 이 표는 그 라운드에서 이 묶음 몫으로 넘길 것이다.

| 자료 | 경로 | 버전 또는 해시 | 읽을 범위 |
|---|---|---|---|
| 평가 대상 코드 | harness/out/task-S9-booking-R1/eval-target-files.md | sha256:45ea1b7f1d14fb46 | 전문. 브랜치 feat/task-s9-booking, 기준 커밋 d111e66. 프로덕션 58개(booking 47, inventory 새 6과 고침 4, shared 고침 1)와 테스트 10파일 73건. 목록과 커밋 여섯은 그 파일 |
...
| R2부터. shared 목록 응답 모델 | backend/src/main/java/com/o2o/shared/PageResponse.java | sha256:aa2feb59977384de | 전문. 프로모션 묶음이 layers.md 3-2로 올린 Page 응답 모델이고 R1 반영 뒤 BookingController가 이것을 쓴다. R1 평가 A-03이 대상 목록 밖이라 못 본 근거. 2026-09-15 개정 4 |
...
### 8-1. 단계별 테스트 목록 (2026-09-12 개정 1 반영. 2026-09-15 개정 4: K26과 K27 추가)
...
| K26 | R1 반영 | hold와 commit과 releaseHeld와 releaseSold가 성공할 때마다 재고 version이 1 오르고 실패(부족, 음수, 0 이하 수량)에서는 그대로다. 선점 전에 조회한 version의 adjust는 VERSION_CONFLICT이고 선점 뒤 version은 통과한다. API 쪽은 K22가 선점 뒤 version 1로 INVENTORY_BELOW_COMMITTED를, version 0으로 VERSION_CONFLICT를 본다 | 11 공통 규칙 43행(재고의 Hold, 확정, 반환도 version 증가), 7절 D-2. R1 평가 B-01 |
| K27 | R1 반영 | 처리 중인 같은 키에 다른 body가 오면 409 REQUEST_IN_PROGRESS가 아니라 409 IDEMPOTENCY_KEY_REUSED이고 Retry-After가 없다. 첫 요청은 그대로 끝나고 다른 body의 거절은 아무것도 남기지 않는다. BOOK-01의 K27과 같은 모양이 2차의 PAY-01 L24와 BOOK-04 L25다 | 11 멱등 규칙 2와 4, 2절 검사 순서 4. R1 평가 A-01 |

K26과 K27은 2026-09-15 개정 4로 들어왔다. 단계 칸이 R1 반영인 것은 이 둘이 8절의 세로 단계가 아니라 R1 평가 반영에서 만들어졌기 때문이다. K26의 도메인 몫은 inventory/domain/DailyInventoryAllocationTest, API 몫은 BookingApiTest의 K22 둘이다. K27은 BookingApiTest이고 잠금 방식은 K17과 같다.
...
| 개정 4 (2026-09-15) | R1 평가 반영. 0절. 사용자가 결정표 6건을 초안대로 확정했고(발언은 초안대로 반영해라) 그 반영이 이 계약에 남긴 것은 5절 R2 행 하나와 8-1의 K26과 K27이다. 근거는 harness/decisions/task-S9-booking-R1.md와 harness/out/task-S9-booking-R1/applied/apply-report.md. 코드는 브랜치 fix/task-s9-booking-r1-apply |
| 마지막 성공 단계 | R1 반영 완료(2026-09-15). 이 계약 몫 셋(A-01, A-03, B-01)이 코드와 테스트에 들어갔고 2차 몫 B-02와 함께 백엔드 전체 테스트가 통과한다. 남은 것은 병합이다. 그 전 기록. 사용자 완료 판단(2026-09-12). 개정 3건 수용과 PR 127 병합 지시. 그 전 기록. 8절 9단계 완료(2026-09-12). BOOK-01부터 BOOK-03과 HoldInventory가 서고 테스트 196건이 통과하며 검증 표와 회고 표가 나왔다. 그 전 기록. 1단계 완료(2026-09-11). 계약과 결정 5건 승인 |
| 실제 사용 시간 (1단계) | 20분 |
| 실제 사용 시간 (2단계) | 4분 |
