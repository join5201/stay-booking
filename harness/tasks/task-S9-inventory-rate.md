...
| V2 | 예약 묶음으로 이월 | soldCount와 heldCount가 음수가 되지 않는다 | I1a |
...
V2를 이월하는 이유 (2026-09-09 결정, join5201). 수량을 내리는 행동이 hold와 commit과 release뿐이고 셋 다 계약 2-2절로 범위 밖이라 이번 묶음에 그 검사에 닿는 경로가 없다. 검사 코드는 DailyInventory에 넣어 두었고 예약 묶음이 그 셋을 붙일 때 이 항목을 받는다. 없는 경로를 억지로 테스트하면 그 테스트가 무엇을 지키는지 흐려진다. 이번 묶음의 테스트는 열하나다.
