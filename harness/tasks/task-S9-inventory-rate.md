...
요구사항 역추적 축에 대한 지시. 입력 팩 2절의 요구사항은 R1부터 R5 다섯이다. 이번 묶음에 걸리는 것은 없다. R1과 R2는 연박 선점과 초과 예약이라 예약 경로가 있어야 하고 그 경로는 2-2절로 범위 밖이다. R5는 스냅샷 동결이라 Booking이 있어야 한다. 평가자는 다섯을 해당 없음으로 적고 미커버로 적지 않는다. 이 Task의 역추적 대상은 2절의 API 9개와 2-1절의 불변식 셋 중 I1과 I2다. I1a는 8-1 V2로 예약 묶음에 이월했으므로 역추적 대상에서 뺀다. 2026-09-11 개정 1. 그 전 문장은 불변식 셋이라 적어 8-1의 이월과 어긋났고 R1 평가 B-02가 짚었다.

R2부터 허용 입력에 넣은 둘은 R1 평가 A-04와 B-04의 반박 근거다. 명세 38행의 미정의 JSON 필드 400은 전역 설정과 앞 묶음 테스트가 이미 고정하는데 둘 다 R1 허용 입력 밖이라 평가자가 볼 수 없었다.

## 6. 정책 적용 (2026-09-11 개정 1. P03 행)

| 정책 ID 또는 쟁점 | 적용할 값 또는 판단 | 상태와 사용자 확인 |
|---|---|---|
| P03 통화와 달력 | 채택. currency는 KRW 고정이고 amount는 정수 원이다. 날짜는 Asia/Seoul 기준 오늘로 판정한다(11 명세 35행). 시각은 UTC이고 날짜는 서울이다. 2026-09-11 개정 1. 그 전 문장은 서버 기준 오늘이라 적어 지역을 비웠고 구현이 UTC로 잘랐다. R1 평가 A-01과 B-01 | 확정. 계약 승인. join5201, 2026-09-09. 지역 명시는 결정표 확정. join5201, 2026-09-11 |
...
### 8-1. 단계별 테스트 목록 (2026-09-11 개정 1. V13과 V14 추가, V2 행 보강)
...
| V2 | 예약 묶음으로 이월 | soldCount와 heldCount가 음수가 되지 않는다. 예약 묶음 계약의 8-1에 필수 항목으로 올린다(2026-09-11 개정 1, R1 평가 B-02) | I1a |
...
| V13 | R1 반영 | 오늘이 UTC 날짜가 아니라 서울 날짜다. UTC 날짜와 서울 날짜가 갈리는 고정 시각에서 서울 기준 어제는 거부되고 서울 기준 오늘은 등록된다 | 11 명세 35행. R1 평가 A-01과 B-01 |
| V14 | R1 반영 | 잠긴 행을 두고 두 트랜잭션이 실제 MySQL에서 경합한다. 잠금이 풀리기 전에는 끝나지 않고 풀린 뒤의 version과 대조해 409를 낸다. 재고와 요금 각각 | 계약 7절 D-2, 06-4 0절, T05의 잠금 몫. R1 평가 A-02 |

V2를 이월하는 이유 (2026-09-09 결정, join5201). 수량을 내리는 행동이 hold와 commit과 release뿐이고 셋 다 계약 2-2절로 범위 밖이라 이번 묶음에 그 검사에 닿는 경로가 없다. 검사 코드는 DailyInventory에 넣어 두었고 예약 묶음이 그 셋을 붙일 때 이 항목을 받는다. 없는 경로를 억지로 테스트하면 그 테스트가 무엇을 지키는지 흐려진다. 이번 묶음의 테스트는 열하나였고 R1 반영으로 V13과 V14가 붙어 열셋이다(2026-09-11 개정 1).

V1은 이월하지 않는다. V2와 달리 상태를 만들 수 있다. soldCount가 0보다 큰 행을 DB에 직접 넣고 그 아래로 조정하면 된다. V2는 전이가 필요하고 V1은 상태가 필요하다는 것이 두 항목의 차이다.
...
| 개정 | 1회. 2026-09-11 R1 평가 반영. 5절 역추적 대상 문장(B-02)과 R2 허용 입력 둘(A-04, B-04), 6절 P03 행의 지역(A-01, B-01), 8-1의 V13과 V14와 V2 행. 근거는 harness/decisions/task-S9-inventory-rate-R1.md. 4절의 dev-ptcf-prompt.v3.md 해시도 현재 파일로 갱신 |
| 마지막 성공 단계 | R1 반영 완료(2026-09-11). 수용 6건이 코드와 문서에 들어갔고 테스트 전부가 통과한다. 남은 것은 R2 재평가와 사용자 최종 완료 판단이다. 그 전 기록. 8절 9단계 완료(2026-09-10). 재고와 요금 API 아홉이 서고 테스트 118건이 통과하며 검증 표와 회고 표가 나왔다. 이 Task의 모델 몫은 끝났고 남은 것은 D-3의 블라인드 평가와 사용자 최종 완료 판단이다 |
| 실제 사용 시간 (1단계) | 15분 |
| 실제 사용 시간 (2단계) | 8분 |
