# task-S9-inventory-rate R2 평가자 A 요청문

최초 작성: 2026-09-14
최종 갱신: 2026-09-14

이 파일은 사용자가 Codex의 새 작업에 그대로 붙여 넣는 요청문이다. 라운드 mvp-eval-2026-09-14(harness/out/mvp-eval-2026-09-14/README.md)의 쌍 2이고 근거는 오늘의 전제 결정 3과 10-4 1절의 전달은 사람이 한다는 규칙이다. A와 B는 각각 다른 새 작업에서 돈다. 한 작업에서 둘 다 하지 않는다. R2다. 4-4절의 확인 항목 여섯은 R1 결정표(harness/decisions/task-S9-inventory-rate-R1.md)의 반영 인계 표에서 같은 자리를 짚은 둘씩을 합쳐 중립 문장으로 옮긴 것이다. 옮긴 사람은 나이고 사용자가 원문과 대조할 수 있게 출처를 여기 적는다. 계약 10절 개정 칸과 5절 두 문장이 R1 지적 번호를 인용하지만 지적 본문은 없다. 계약은 허용 입력이라 그대로 간다.
...
열기 전에 `node harness/out/task-S9-inventory-rate-R2/verify-eval-workspace.mjs`를 그 폴더에서 돌려 PASS를 확인한다. FAIL이면 붙여 넣지 않는다. 평가자 B의 요청문은 같은 폴더의 eval-request-B.md다.

리포트는 `harness/reviews/task-S9-inventory-rate-R2-A.md`에 저장된다. 그 파일이 생겼는지 붙여 넣은 뒤에 본다.
...
| 자료 | 경로 | 해시 | 읽을 범위 |
|---|---|---|---|
| 평가 대상 코드 목록 | harness/out/task-S9-inventory-rate-R1/eval-target-files.md | sha256:f01d6c51b6bcc3e7 | 전문 |
...
| 앞 묶음의 미정의 필드 거절 테스트 | backend/src/test/java/com/o2o/catalog/api/CatalogApiTest.java | sha256:700d593eed9f3741 | C6 155행 |
| 생성자가 낸 실행 결과 | harness/out/task-S9-inventory-rate-R1/step5, step6, step6-2와 harness/out/task-S9-booking-lifecycle-R1/step9의 JUnit XML | 없음 | JUnit XML과 http-calls.txt와 요약. 단계 사본은 지난 라운드 시점 코드의 결과이고 기준 커밋 코드로 돈 결과는 step9의 XML 48개다. step6과 step6-2의 http-calls.txt는 살아 있는 서버에 친 기록이다 |
...
계약 9절이 지정한 것은 T03과 T04와 T05 전부, T06과 T07 부분이다(예약 몫은 예약 쌍이 본다). 계약 8-1절의 V1부터 V14가 근거 테스트다. 각각에 통과, 실패, 미실행 셋 중 하나를 적는다. 실행하지 않은 것을 통과로 적지 않는다.

### 4-4. 이번 라운드에서 특히 볼 것

아래 여섯은 축을 대신하지 않는다. 4-1과 4-2와 4-3을 전부 본 뒤 이 여섯에 대해 각각 확인 결과를 한 줄씩 적는다. 지적이면 상세 표에 넣고 지적이 아니면 확인함으로 적는다.

1. 숙박 날짜의 오늘을 어느 시간대로 판정하는가. 자정 전후의 경계를 테스트가 짚는가
2. 재고와 요금의 잠금이 실제 MySQL 두 트랜잭션의 경합 테스트로 확인되는가. 한 트랜잭션 안의 흉내가 아닌가
3. RATE-02의 과거 날짜 수정 거부와 RATE-03과 RATE-04의 과거 날짜 조회 허용에 각각 테스트가 있는가
4. I1a(soldCount와 heldCount는 음수가 될 수 없다)를 직접 짚는 테스트가 있는가. 지금 코드에서 그 규칙을 어디가 지키는가
5. 도메인 이벤트가 커밋 뒤에 구독자에 닿는가. 예약 묶음의 구독자가 있어 지금은 직접 볼 수 있다
6. 미정의 JSON 필드와 명세에 없는 Query 파라미터가 400인가. 허용 입력의 전역 설정과 앞 묶음 테스트로 판정한다
...
| `SPRING_DATASOURCE_URL=jdbc:mysql://127.0.0.1:3307/o2o_booking_test JAVA_HOME=C:/Users/user/.jdks/ms-21.0.11 ./backend/gradlew.bat -p backend test` | 테스트 전부. 저장소 루트에서. 419건이 기준선이다 |
| `node harness/tools/check.mjs g1 <소스> --type code --artifact <junit.xml>` | 게이트 |

테스트 DB는 `o2o_booking_test`다. 설정 파일 기본값은 다른 DB라 명령 앞에 환경변수를 붙인다. PowerShell이면 `$env:SPRING_DATASOURCE_URL='jdbc:mysql://127.0.0.1:3307/o2o_booking_test'`와 `$env:JAVA_HOME='C:/Users/user/.jdks/ms-21.0.11'`를 먼저 주고 `.\backend\gradlew.bat -p backend test`를 돌린다. 설정 파일은 고치지 않는다.

바꿔도 되는 것은 빌드 산출물과 로그와 테스트 DB `o2o_booking_test` 안의 표뿐이다. 소스와 테스트와 설정은 어떤 파일도 고치지 않는다. 컨테이너를 내리지 않는다. 다른 세션이 같은 컨테이너의 다른 DB를 쓴다.
...
지적 ID는 `S9-R2-A-{원본 번호 2자리}`로 붙인다. 위치는 파일과 행 번호로 적는다.

리포트에 실제로 읽은 파일과 그 sha256을 남긴다.

리포트를 `harness/reviews/task-S9-inventory-rate-R2-A.md`에 저장한다. 이 파일 하나만 만든다.
