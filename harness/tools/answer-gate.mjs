...
// 한 턴에서 막는 횟수를 여기서 직접 제한한다. 막힌 답변이 또 막히면
// 세션이 빠져나오지 못한다.
//
...
//   답변 형식 검사 실패. 앞 답변을 다시 쓰지 말고 빠진 것만 이어서 낸다 (CLAUDE.md 2-1, harness/prompts/answer-format.md)
//     [answer.result-section]  40  마지막 절 제목이 결과가 아니다 (R4). 실제: 다음 단계
//   막힌 뒤 모델은 앞 글 뒤에 이어 쓴다. 지운 뒤 다시 쓰지 않는다 (2026-09-08 실측)
//
// 안전 규칙 셋
//   1. 한 턴에서 막는 횟수는 ANSWER_GATE_MAX_BLOCKS번까지다. 기본 1. 턴은 prompt_id로 가른다
...
function main() {
  ...
  // 실패다. 한 턴을 두 번 막지 않는다. 그것이 무한 반복이 나는 유일한 자리다.
  // 턴은 prompt_id로 가른다. session_id로 가르면 세션당 한 번밖에 못 고친다 (2026-09-08 실측)
  const key = input.prompt_id || input.session_id || 'unknown';
  const state = loadState();
  const used = state[key] || 0;
  if (used >= MAX_BLOCKS) {
    console.error(`답변 형식 검사가 또 실패했지만 이 턴의 차단 한도 ${MAX_BLOCKS}을 이미 썼다. 막지 않는다`);
    return 0;
  }
  state[key] = used + 1;
  // 오래된 턴 기록은 버린다. 안 그러면 파일이 무한히 큰다
  ...
  // 정지를 막으면 모델이 앞 글 뒤에 이어 쓴다. 다시 답하라고 하면 답변이 통째로 두 번 나온다.
  // 그래서 빠진 것만 채우라고 적는다 (2026-09-08 실측, harness/docs/10-9 7절)
  console.error('답변 형식 검사 실패. 앞 답변을 다시 쓰지 말고 빠진 것만 이어서 낸다 (CLAUDE.md 2-1, harness/prompts/answer-format.md)');
