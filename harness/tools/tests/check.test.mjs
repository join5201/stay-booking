...
test('g1 doc 통과. 백틱 두 개 코드 구간 안의 링크 모양은 링크가 아니다', () => {
  // 렌더 HTML <code>[x](#없는-절)</code>. 백틱 하나짜리만 걸러서 고치기 전에는 실패했다
  const r = run(() => g1(anchorPair('``[x](#없는-절)``'), { type: 'doc' }));
  assert.equal(r.code, 0);
});

test('g1 doc 실패. 역슬래시로 푼 백틱 사이의 링크는 진짜 링크다', () => {
  // 렌더 HTML `<a href="#없는-절">x</a>`. 코드로 읽으면 깨진 링크를 놓친다
  const r = run(() => g1(anchorPair('\\`[x](#없는-절)\\`'), { type: 'doc' }));
  assert.equal(r.code, 1);
...
test('g1 doc 실패. 표 칸의 홀로 선 백틱은 다음 행의 백틱과 코드 구간을 이루지 않는다', () => {
  ...
  const f = prep('g1-doc-pass.md', (t) => t
    .replace('| --type doc\\|api\\|code | 값이 셋이라도 두 칸이다 |', '| --type ` 하나 | 값이 셋이라도 두 칸이다 |')
    .replace('| --dry | 쓰지 않고 결과만 낸다 |', '| --dry | [x](#없는-절) `코드` |'));
  assert.match(fs.readFileSync(f, 'utf8'), /--type ` 하나[\s\S]*\[x\]\(#없는-절\) `코드`/); // 치환이 헛돌면 공짜로 통과한다
  const r = run(() => g1(f, { type: 'doc' }));
  assert.equal(r.code, 1);
  assert.match(r.out, /\[link\.anchor\] +\d+ +절 주소가 대상 파일에 없다: #없는-절$/m);
});
...
test('scopeKind. 경로 앞자리가 파일 성격을 정한다', () => {
  ...
  assert.equal(scopeKind(path.join(ROOT, '.claude/skills/round/SKILL.md')), 'harness-doc');
});
