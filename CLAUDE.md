...
최초 작성: 2026-09-07
최종 갱신: 2026-09-25 (1절 범위 행에 EC2 한 대 배포와 로그 조회 포함. 결정 D1, 이슈 206. 그 전 2026-09-23 3절 .claude 행과 2-1절 포인터의 스킬 경로를 .claude/skills 바로 아래로. 이슈 192)
...
| 프론트 | Next.js |
| 범위 | 로컬 개발과 검증, EC2 한 대 배포와 로그 조회까지. 실서비스 운영 제외(2026-09-25 D1) |
...
### 2-1. 답변 형식 (2026-09-08 신설, 같은 날 규격 분리. 2026-09-14 R5. 2026-09-21 스킬. 2026-09-23 스킬 경로)
...
| R5 | 사용자에게 결정 쟁점을 구하는 턴은 쟁점 하나를 여섯 칸 양식(결정, 기존 기준과의 관계, 이유, 근거, 조건과 한계, 상태)으로 낸다. 결과 항목은 그 양식이 대신한다. 근거는 원문 구절과 위치이고 확인 못 한 것은 미확인이라고 적는다 |

2026-09-08 사용자 지적이다. 하네스 작업은 산출물이 많아서 만들었다는 보고만 하면 그래서 뭐가 좋아졌는지를 매번 되묻게 된다. R5는 2026-09-14 사용자 제안이다. 결정을 구하는 답변에 결과 절과 ReAct와 남은 단계 표가 겹쳐 붙어 길어지고 근거가 흩어졌다. 등급 A와 B와 C와 S, 답변 골격, 결과 절 다섯 행, 게이트 검사 목록은 harness/prompts/answer-format.md에 있다. 규칙 다섯 줄만 여기 남긴 것은 이 파일이 세션마다 자동으로 읽히고 그쪽은 아니기 때문이다(harness/docs/10-9 2-1절). 결정을 구하는 턴은 .claude/skills/approval-request 스킬이 그 3-1절을 연다(2026-09-21).

## 3. 파일 규약 (2026-09-08 개편. 2026-09-23 스킬 경로)
...
| README.md | 저장소 소개 |
| .claude/ | 도구 설정과 스킬 폴더. settings.json, hooks, skills/ 바로 아래 절차 스킬 넷(round, progress-row, approval-request, guarded-paths). 플러그인 묶음으로 싸지 않는다. 이유는 .claude/skills/README.md 동작 조건 절 |
...
### 3-1. project-sync 반입 절차 (2026-09-08 신설. 2026-09-10 본문을 10-15로 내림. 2026-09-21 스킬)

본문은 harness/docs/10-15 2절이다. project-sync 아래를 만지거나 반입 후보를 만들 때 guarded-paths 스킬이 그 절을 연다. settings.json 수정도 같은 스킬이다.
...
진행 기록 규칙: 한 단계나 한 행이 끝나면 harness/state/progress.md에 한 행을 추가한다. 열두 칸이고(2026-09-10 이전 행은 아홉 칸이다) 결과 칸은 pending, drafted, reviewed, applied, done, halted 여섯 중 하나다. done은 사용자 최종 승인 행에서만 쓴다(HR5). 기존 행은 고치지 않는다. 실패 원인 칸이 재요청과 재평가 프롬프트에 넣는 사유의 원천이다(HR3). 세션이 여럿이면 파일의 마지막 행이 아니라 그 Task의 마지막 행을 읽는다. 행을 추가할 때 머리글의 최종 갱신 줄은 건드리지 않는다. progress.md와 troubleshooting.md와 knowledge.md는 .gitattributes에서 merge=union이다. 로컬 병합에서는 두 세션의 추가가 충돌 없이 둘 다 남는다. GitHub의 서버 병합에는 이 설정이 걸리지 않아 같은 파일에 행을 더한 PR 둘 중 뒤엣것이 충돌로 막힌다. 그래서 4-1의 5단계가 PR 전에 브랜치에서 origin/main을 병합하게 한다. 순서는 어느 쪽이든 보장되지 않으므로 날짜시각 칸으로 읽는다. 앞 행이 틀렸으면 고치지 말고 새 행의 대체 칸에 그 행의 날짜시각을 적는다.

### 4-1. 작업 흐름 (2026-09-08 신설. 2026-09-10 settings 이유 표를 10-15로 내리고 5단계 신설. 2026-09-11 5단계에 union 검사. 2026-09-16 7단계 신설)
...
| 4 | 검사를 돌리고 결과를 기록한다 |
| 5 | 브랜치에서 origin/main을 병합한다. 기록 파일의 union 병합은 로컬에서만 걸린다. 병합한 뒤 node harness/tools/check.mjs union .gitattributes 를 돌린다. union이 두 번 남긴 행을 잡는다 |
| 6 | PR을 올린다. 본문에 Closes로 이슈를 잇는다 |
| 7 | 사용자가 병합을 승인하면 done 행과 README 상태 행을 그 브랜치의 마지막 커밋으로 얹고 병합한다. 행에는 병합 커밋 해시 대신 PR 번호를 적는다. 기록만을 위한 브랜치는 열지 않는다. 이미 병합된 뒤라면 다음 작업 브랜치의 첫 커밋으로 얹는다 |
...
5단계는 2026-09-10에 넣었다. GitHub는 병합 가능 여부를 계산할 때 저장소의 .gitattributes를 적용하지 않는다. 그래서 기록 파일에 행을 더한 PR 둘을 차례로 병합하면 뒤엣것이 충돌로 막힌다. 브랜치에서 먼저 병합해 두면 그 병합은 로컬 git이 하므로 union이 걸리고, GitHub는 이미 합쳐진 결과를 받는다. PR을 올린 뒤 main이 또 움직여 충돌로 바뀌면 같은 단계를 한 번 더 한다. 근거와 재현은 harness/docs/10-10 6-1절.

7단계는 2026-09-16에 넣었다. 그 전에는 병합이 끝난 뒤 병합 커밋 해시를 담은 done 행을 적으려고 chore/merge-{PR}-record 브랜치를 열고 PR을 하나 더 올렸다. 어느 문서에도 없는 관행이었고 스물세 번 그랬다. 병합 커밋 140개 중 23개와 이슈와 브랜치와 PR과 병합의 사용자 왕복 23번이 그 몫이었다. 병합 해시는 GitHub의 PR 페이지가 갖고 있고 버전 근거는 원래 검사 스크립트의 sha256이지 커밋 해시가 아니다(1절, HR2). 사용자 승인은 병합 전 대화에서 나오므로 승인 뒤 브랜치에 적어도 HR5는 그대로다. 2026-09-16 사용자 결정이고 근거는 이슈 180.

settings.json의 deny와 allow를 그렇게 정한 이유는 harness/docs/10-15 3절에 있다. .claude/settings.json을 고칠 때 연다.
...
### 5-1. 합리화 신호 (2026-09-10 신설. 2026-09-11 union 행 추가)

5절이 무엇을 금지하는지 적는다면 이 표는 그 금지를 어기기 직전에 머릿속에 뜨는 문장을 적는다. 규칙만 적어 두면 샌다. 답변 형식 R1부터 R4가 규칙으로만 있을 때 두 답변 연속으로 샜고, 그때 배운 것이 문구를 강화하는 대신 지킬 수밖에 없는 자리를 만드는 것이었다(harness/prompts/answer-format.md 머리).

행은 전부 harness/state/troubleshooting.md에 실제로 기록된 것에서 뽑았다. 지어내지 않는다. 새 행은 같은 원인이 두 번 난 뒤에 올린다. 근거와 대조 과정은 harness/docs/10-14 6절.

| 머릿속 문장 | 실제 |
|---|---|
| 사람이 읽기 좋게 한마디 덧붙이자 | 검사기가 읽는 칸은 값만 담는 자리다. 그 한마디가 값의 일부가 된다 |
| 명령 둘을 이어서 한 줄로 주자 | 사용자 셸은 Windows PowerShell 5.1이다. 그 판에는 파이프라인 체인 연산자가 없다 |
| git status가 신규라고 하겠지 | 같은 작업 트리를 세션 여럿이 쓴다. 손대기 전에 status와 열린 PR 목록을 먼저 본다 |
| 여는 김에 문체도 다듬자 | N7과 N8이다. 범위 밖 변경이 섞이면 어느 변경이 무엇을 깼는지 못 가린다 |
| 결과 파일이 생겼으니 통과다 | 실패한 빌드 로그도 파일이다. 파일의 존재가 아니라 안의 숫자를 읽는다 |
| 테스트가 다 통과하니 동작한다 | 테스트는 자기 설정 파일을 따로 갖는다. 28건이 통과해도 기동이 죽은 적이 있다 |
| 문서가 규칙을 안 지켰나 보다 | 같은 실패가 여럿이면 산출물이 아니라 검사기의 범위를 먼저 본다 |
| 5단계를 밟았으니 기록 파일은 알아서 합쳐졌겠지 | union은 양쪽이 같은 구간을 건드리면 충돌 대신 두 판을 다 남긴다. 네 번 났다. 병합 뒤 union 검사를 돌린다 |
