---
name: guarded-paths
description: harness/project-sync 아래 원본 사본을 반입하거나 그 아래 낡은 경로를 발견했을 때, 그리고 .claude/settings.json의 deny와 allow와 hooks를 고칠 때 쓴다. 두 절차의 본문은 harness/docs/10-15 2절과 3절이다.
paths:
...
| deny와 allow를 그렇게 정한 이유와 안 넣은 것(defaultMode, ask, hooks)의 이유를 먼저 읽는다 | harness/docs/10-15 3절과 3-1절 |
