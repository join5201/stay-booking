...
상태: 구현용 초안  
최초 작성: 2026-09-07  
최종 갱신: 2026-09-16 (상태 전이와 시간 경계 절, 결제 접수 항목, T18과 T19. 승인 판정 기준을 승인 기록 시각으로. 이슈 184)
...
| HELD | 승인 기록 시각 < expiresAt | CONFIRMED | 한 건 승인, 선점 재고를 판매 재고로 이동 |
| HELD | 승인 기록 시각 >= expiresAt인 승인 처리 | EXPIRED | TTL_EXPIRED, 선점 재고 반환, 승인액 환불 |
...
승인과 만료가 경합하면 같은 예약에 대한 상태 전이를 직렬화한다. 승인은 서버가 승인을 기록한 시각을 expiresAt과 비교해 판단하고, 만료 스캔과 실패 처리는 처리 시각으로 판단한다. 승인 기록 시각이 정확히 expiresAt이면 만료다. 만료 스캔이 만료 전 승인 기록을 보면 확정하고 이후 기록을 보면 환불하고 만료한다. 어느 처리가 먼저 잠그든 결말이 같다. 외부 결제사나 클라이언트가 제공한 결제 시각으로 과거로 돌려 승인하지 않는다.
...
- GET에 잠시 HELD가 보이더라도 승인 기록 시각이 expiresAt 이상이면 승인할 수 없다. 응답 상태나 화면 카운트다운이 승인 가능성을 보장하지 않는다.
...
| <a id="t18"></a>T18 | 승인 기록 시각이 expiresAt과 정확히 같은 결제 승인 | EXPIRED, TTL_EXPIRED, 승인액 환불 |
| <a id="t19"></a>T19 | 만료 스케줄러와 승인 경합 | 확정 또는 만료와 환불 중 승인 기록 시각으로 정해지는 한 경로만 반영. 처리 순서와 무관 |
