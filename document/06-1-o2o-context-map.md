...
최초 작성: 2026-09-02
최종 갱신: 2026-09-09 v2 R1 반영 (평가 지적 2건. R8 동기와 비동기 구분, 공유 커널 판정 근거 기재). 2026-09-08 v2 (08-3 결정 반영: R6 흐르는 것에 SettlePayment와 조회 API 둘 추가, R6 근거 문구 정정, 구독자 없는 이벤트 20개, 정책 번호를 04-5 v2 신 번호로, 도식에 T2)
...
| R8 | 카탈로그, 재고와 요금, 프로모션에서 검색으로 | 검색과 조회용 읽기 | 동기 읽기 (2026-09-09 R1 반영. S8-R1-A-07) | 읽기 모델 (Published Language 성격) | 발견 4. 커맨드 없음 |
...
| 후보 | 판정 | 근거 |
|---|---|---|
| ID 값 타입 (PropertyId, RoomTypeId, BookingId, UserId 등) | 적합 | 3기준 통과. 근거는 표 아래 (2026-09-09 R1 반영. S8-R1-B-04) |
...
ID 값 타입이 3기준을 통과하는 이유 (2026-09-09 R1 반영 신설. S8-R1-B-04). 기준 이름만 적어 두면 판단 과정을 재구성할 수 없다는 지적을 받아 셋을 각각 적는다.

| 기준 | 왜 통과하나 |
|---|---|
| 안정 | ID 타입이 담는 것은 식별자 값 하나이고 그 의미는 어느 컨텍스트에서도 같다. 도메인 규칙이 바뀌어도 식별자의 뜻은 바뀌지 않는다. 지금까지 판이 스무 번 넘게 오르는 동안 한 번도 안 바뀌었다 |
| 보편 | 여섯 노드 중 다섯이 남의 식별자를 참조한다. 예약은 RoomTypeId를, 재고는 RoomTypeId를, 검색은 PropertyId를 쓴다. 공유하지 않으면 같은 타입을 노드마다 다시 정의하게 된다 |
| 단순 | 값 하나를 감싸는 VO다. 행동이 없고 다른 타입에 의존하지 않는다. 그래서 이 타입이 바뀔 일이 없고 바뀌어도 파급이 없다 |

04-6의 결론을 인용하는 것으로 근거를 대신하지 않는다. 04-6은 허용 입력에 없을 수 있고 그러면 이 판정을 문서만 보고 확인할 수 없다.
